...
static void display_elf_hedaer(Elf32_Ehdr eh) {
  ...
  if (ef & EF_ARM_RELEXEC) ftrace_write(",RELEXEC ");

  if (ef & EF_ARM_HASENTRY) ftrace_write(",HASENTRY ");

  if (ef & EF_ARM_INTERWORK) ftrace_write(",INTERWORK ");

  if (ef & EF_ARM_APCS_26) ftrace_write(",APCS_26 ");

  if (ef & EF_ARM_APCS_FLOAT) ftrace_write(",APCS_FLOAT ");

  if (ef & EF_ARM_PIC) ftrace_write(",PIC ");

  if (ef & EF_ARM_ALIGN8) ftrace_write(",ALIGN8 ");

  if (ef & EF_ARM_NEW_ABI) ftrace_write(",NEW_ABI ");

  if (ef & EF_ARM_OLD_ABI) ftrace_write(",OLD_ABI ");

  if (ef & EF_ARM_SOFT_FLOAT) ftrace_write(",SOFT_FLOAT ");

  if (ef & EF_ARM_VFP_FLOAT) ftrace_write(",VFP_FLOAT ");

  if (ef & EF_ARM_MAVERICK_FLOAT) ftrace_write(",MAVERICK_FLOAT ");
...
static void display_section_headers(int fd, Elf32_Ehdr eh,
  ...
  ftrace_write("========================================\n");
  ftrace_write(
      " idx offset     load-addr  size       algn"
      " flags      type       section\n");
  ftrace_write("========================================");
  ftrace_write("========================================\n");
