...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  // *-------------------------------------------------------------------------------------------------------------------* //
  INSTPAT("??????? ????? ????? ??? ????? 11011 11", jal    , J, s->dnpc = s->pc + imm; TRACE_JAL(s, rd, imm); R(rd) = s->pc + 4);
  INSTPAT("??????? ????? ????? 000 ????? 11001 11", jalr   , I, s->dnpc = (src1 + imm) & ~(word_t)1; TRACE_JALR(s, rd, imm, src1); R(rd) = s->pc + 4);
  // *-------------------------------------------------------------------------------------------------------------------* //
